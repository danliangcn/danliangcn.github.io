### Problem 1
pyth <- read.table("pyth/exercise2.1.dat", header = T)
...
### Problem 2
library(foreign)
childiq <- read.dta("child.iq/child.iq.dta")
...
### problem 3
beauty <- read.csv("beauty/ProfEvaltnsBeautyPublic.csv",header = T)
...
# optional: robust standard error
library(lmtest);library(sandwich)
for(i in c("HC3", "const", "HC", "HC0", "HC1", "HC2", "HC4", "HC4m", "HC5")) {
  print(i)
  print(coeftest(fit3wls, vcov = vcovHC(fit3wls, type = i)))
}
coeftest(fit3wls, vcov = vcovHC(fit3wls, type = i))

# replicate subset regression
# males
fit3wls <- lm(courseevaluation~btystdave+minority+nonenglish+tenuretrack+lower+onecredit,weights = didevaluation,data = beauty[beauty$female==0,])
summary(fit3wls)
...
summary(beauty)
weighted.mean(x=beauty$courseevaluation,w=beauty$didevaluation)
